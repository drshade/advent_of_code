...
-- Flat map! (isn't this the same as =<< ?)
   ...
   in case continuous (start, startv) (stop, stopv) of
        ...
        Continue ->
          let mid = (start + stop) `div` 2
           in -- Split the region and fork the search
              seeker start mid produce continuous
                ++ seeker mid stop produce continuous

-- A fast algorithm to find all combinations of a list
